...
class ScriptArguments:
    ...
    model_name_or_path: Optional[str] = field(
        default="RLHFlow/Llama3-v2-iterative-DPO-iter3",
        metadata={"help": "the location of the SFT model name or path"},
    )
    dataset_name_or_path: Optional[str] = field(
        default='./train_prompt_online/online_hh_train.json',
...
ds = load_dataset("json", data_files=script_args.dataset_name_or_path, split="train[60%:]")
ds = ds.map(
    lambda x: {
...
prompts1 = ds[0]["prompt"]
outputs1 = llm.generate(prompts1, sampling_params=sampling_params1, use_tqdm=True)
for i in outputs1[0].outputs:
    print(i)
...
for i, output in enumerate(outputs):
    tmp_data = {"prompt": ds[i][script_args.dataset_key], "responses": ds[i]['responses']+[out.text for out in output.outputs]}
